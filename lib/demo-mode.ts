...
import arrangementsData from '@/data/mock/arrangements.json'
import occasionsData from '@/data/mock/occasions.json'
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetArrangements') || query.includes('nodeArrangements')) {
      return loadMockData('arrangements.json')
    }

    if (query.includes('GetOccasions') || query.includes('nodeOccasions')) {
      return loadMockData('occasions.json')
    }
