...
async function getTestimonial(path: string): Promise<DrupalTestimonial | null> {
  try {
    ...
    const { data } = await apolloClient.query<TestimonialByPathData>({
      query: GET_TESTIMONIAL_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) {
    return { title: 'Testimonial Not Found | Florist' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).clientLocation && (
                    <div>
                      <dt className="text-sm text-gray-500">Client Location</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).clientLocation}</dd>
                    </div>
                  )}
                  {(item as any).occasionType && (
                    <div>
                      <dt className="text-sm text-gray-500">Occasion Type</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).occasionType}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
