...
interface ArrangementByPathData {
  route: {
    entity: DrupalArrangement
  } | null
}
...
async function getArrangement(path: string): Promise<DrupalArrangement | null> {
  try {
    ...
    const { data } = await apolloClient.query<ArrangementByPathData>({
      query: GET_ARRANGEMENT_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/arrangements/${slug.join('/')}`
  const item = await getArrangement(path)

  if (!item) {
    return { title: 'Arrangement Not Found | Florist' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).arrangementSize && (
                    <div>
                      <dt className="text-sm text-gray-500">Size</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).arrangementSize}</dd>
                    </div>
                  )}
                  ...
                  {(item as any).bestFor && (
                    <div>
                      <dt className="text-sm text-gray-500">Best For</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).bestFor}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
